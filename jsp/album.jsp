...
<%
	...
	if(u_no !=null){
		...
		if(u_no.equals(user.u_no)){  //相册主人浏览
			albumList = controler.getAlbumList(u_no);//相册主人好友浏览
		}
		else{
			albumList = controler.getAlbumListByAccess(u_no,user.u_no);
		}
		int pageNo = 1;
		int span = 1;	//每页显示一张
...
%>
	...
	<%
		...
		if(xid!=null){			//如果选择了相册
			int albumSize = controler.getAlbumSize(xid);
			if(pageNo > albumSize) //如果删除了最后一张，则返回显示前一张
				{
					pageNo -= 1;
				}
			ArrayList<PhotoInfo> photoList = controler.getPhotoInfoByAlbum(xid,pageNo,span);//	获取图片列表
			//int albumSize = controler.getAlbumSize(xid);
	...
	if(u_no.equals(user.u_no)){
		int accessLevel=controler.getAlbumAccess(xid);
	%>
	...
	%>
		...
		%>
					<a href="PhotoServlet?action=deletePhoto&p_id=<%=pi.p_id %>&xid=<%=xid %>&pageNo=<%=pageNo %>&u_no=<%=u_no %>&listSize=<%= albumList.size()%></a>">[删除]</a>
		<%
						}
		%>
						</td>
	...
	<%
			ArrayList<P_Comments> cmtList = controler.getPhotoComment(p_id);
			if(cmtList.size() == 0){

...
public class PublishActivity extends Activity
{
	...
	"更新心情", "发布日志", "拍照上传" };
	int[] imgIds = { R.drawable.p_status, R.drawable.p_diary,
			R.drawable.p_shoot };
	String uno = null; // 存放
		...
		lvPublish.setOnItemClickListener(new OnItemClickListener()
		{
					...
					int position, long id)
			{
				...
				case 0: // 更新心情
					...
					break;
				case 1: // 发表日志
					Intent intent1 = new Intent(PublishActivity.this,
							PublishDiaryActivity.class);// 创建Intent
					intent1.putExtra("uno", uno); // 设置Intent的Extra字段
					startActivity(intent1);
					break;
				case 2: // 拍照上传
					Intent intent2 = new Intent(PublishActivity.this,
							ShootActivity.class);// 创建Intent
	...
	public void updateStatus()
	{
		new Thread()
		{
			...
			{
				Looper.prepare();
				EditText etStatus = (EditText) dialog_view
						.findViewById(R.id.etInputStatus);
				String status = etStatus.getEditableText().toString().trim(); // 获得心情内容
				if (status.equals(""))
				{ // 如果输入的心情为空
					pd.dismiss();
					Toast.makeText(PublishActivity.this, "请输入更新的心情",
							Toast.LENGTH_LONG).show();// 输出提示
					Looper.loop();
					return;
				}
				...
				try
				{
					...
					pd = ProgressDialog.show(PublishActivity.this, "正在连接网络",
							"请稍候", true);
					String reply = mc.din.readUTF(); // 从服务器返回的回复
					pd.dismiss();
					if (reply.equals("<#STATUS_SUCCESS#>"))
					{ // 心情更新成功
						Toast.makeText(PublishActivity.this, "心情更新成功！",
								Toast.LENGTH_LONG).show();
						Looper.loop();
					} else
					{ // 心情更新失败
						Toast.makeText(PublishActivity.this, "心情更新失败！",
								Toast.LENGTH_LONG).show();
						Looper.loop();
					}
				} catch (Exception e)

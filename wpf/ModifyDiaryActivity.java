...
public class ModifyDiaryActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		btnModifyDiary.setOnClickListener(new View.OnClickListener()
		{
			...
			{
				String modifyTitle = etModifyTitle.getEditableText().toString()
						.trim();
				String modifyContent = etModifyContent.getEditableText()
						.toString().trim();
				if (modifyContent.equals("") || modifyTitle.equals(""))
				{ // 如果内容或标题为空
					Toast.makeText(ModifyDiaryActivity.this, "请将标题或内容填写完整！",
		...
		btnModifyBack.setOnClickListener(new View.OnClickListener()
		{
			...
			public void onClick(View v)
			{
				Intent intent = new Intent(ModifyDiaryActivity.this,
		...
		new Thread()
		{
			public void run()
			{
				...
				try
				{
					...
					String modifyTitle = etModifyTitle.getEditableText()
							.toString().trim();
					String modifyContent = etModifyContent.getEditableText()
							.toString().trim();
					String msg = "<#MODIFY_DIARY#>" + diaryInfo[0] + "|"
							+ modifyTitle + "|" + modifyContent;
					mc.dout.writeUTF(msg);
					String reply = mc.din.readUTF();
					if (reply.equals("<#MODIFY_DIARY_SUCCESS#>"))
					{
						Toast.makeText(ModifyDiaryActivity.this, "日志修改成功！",
								Toast.LENGTH_LONG).show();
						Looper.loop();
					} else
					{
						Toast.makeText(ModifyDiaryActivity.this, "修改失败，请重试！",
								Toast.LENGTH_LONG).show();
						Looper.loop();
					}
				} catch (Exception e)

...
public class UploadActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		{ // 为按钮添加监听器
					...
					{ // 重写onClick方法
						final View dialog_view = LayoutInflater.from(
								UploadActivity.this).inflate(
								R.layout.new_album, null); // 获得对话框视图
								...
								.setPositiveButton(R.string.btnOk,
										new DialogInterface.OnClickListener()
										{
											...
											public void onClick(
													...
													int which)
											{
												EditText et = (EditText) dialog_view
														.findViewById(R.id.etAlbumName);
												newAlbum = et.getEditableText()
														.toString().trim();
												if (newAlbum.equals(""))
												{ // 如果
													Toast.makeText(
		...
		btnUpload.setOnClickListener(new View.OnClickListener()
		{
			...
			{ // 点下上传按钮后出发
				...
				if (position < 0)
				{
					Toast.makeText(UploadActivity.this, "您还没有选择上传相册！",
				...
				{ // 没有输入任何消息
					Toast.makeText(UploadActivity.this, "请输入照片 名称",
							Toast.LENGTH_LONG).show();
					return;
				}
				pd = ProgressDialog.show(UploadActivity.this, "请稍候",
						"正在上传图片...", true, true);
		...
		new Thread()
		{
			public void run()
			{
				try
				{
					...
					{ // 如果无相册
						Toast.makeText(UploadActivity.this, "您还没有创建任何相册！",
								Toast.LENGTH_LONG).show();
			...
			public void run()
			{
				...
				try
				{
					...
					{ // 如果创建成功
						...
						Toast.makeText(UploadActivity.this, "相册创建成功！",
								Toast.LENGTH_LONG).show();
						Looper.loop(); // 执行本线程中的消息队列
					} else
					{
						Toast.makeText(UploadActivity.this, "相册创建失败，请重试！",
								Toast.LENGTH_LONG).show();
						Looper.loop();
					}
				} catch (Exception e)
		...
		new Thread()
		{
			public void run()
			{
				...
				try
				{
					...
					{ // 上传成功
						pd.dismiss();
						Toast.makeText(UploadActivity.this, "照片上传成功！",
								Toast.LENGTH_LONG).show();
						Looper.loop();
					} else
					{
						pd.dismiss();
						Toast.makeText(UploadActivity.this, "照片上传失败！",
								Toast.LENGTH_LONG).show();
						Looper.loop();
					}
				} catch (Exception e)

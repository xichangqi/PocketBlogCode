...
public class MyAlbumListActivity extends Activity
{
	...
	String[] accessOptions = { "公开", "好友可见", "仅个人可见" };
	...
	BaseAdapter ba = new BaseAdapter()
	{
		...
		public View getView(int position, View arg1, ViewGroup arg2)
		{
			...
			tvName.setLayoutParams(new LinearLayout.LayoutParams(170,
			...
			btnSee.setLayoutParams(new LinearLayout.LayoutParams(60,
					LayoutParams.WRAP_CONTENT));
			ll2.addView(btnSee);
			...
			btnChangeAccess.setTextAppearance(MyAlbumListActivity.this,
					R.style.button); // 设置按钮样式
			btnChangeAccess.setOnClickListener(listenerToAcess); // 添加监听器
			btnChangeAccess.setLayoutParams(new LinearLayout.LayoutParams(80,
					LayoutParams.WRAP_CONTENT));// 设置布局参数
	...
	{ // 点下查看按钮后触发的监听器
		...
		{ // 重写onClick方法
			Intent intent = new Intent(MyAlbumListActivity.this,
					AlbumActivity.class); // 创建Intent对象
		...
		new Thread()
		{
			public void run()
			{
				...
				try
				{
					...
					{ // 判断相册列表是否为空
						Toast.makeText(MyAlbumListActivity.this, "您还没有上传过照片",
	...
	public void showMyDialog()
	{
		new AlertDialog.Builder(this)
				.setSingleChoiceItems(accessOptions, newAccess,
						new OnClickListener()
						{
							@Override
		...
		new Thread()
		{
			public void run()
			{
				...
				try
				{
					...
					String msg = "<#CHANGE_ALBUM_ACCESS#>" + albumToChange
							+ "|" + accessToChange;
					mc.dout.writeUTF(msg);
					String reply = mc.din.readUTF(); // 接收服务器反馈
					if (reply.equals("<#ALBUM_ACCESS_SUCCESS#>"))
					{ // 更新权限成功
						Toast.makeText(MyAlbumListActivity.this, "相册权限更新成功！",
								Toast.LENGTH_LONG).show();
						Looper.loop(); // 执行消息队列中的消息
					} else
					{ // 更新权限失败
						Toast.makeText(MyAlbumListActivity.this, "相册权限更新失败！",
								Toast.LENGTH_LONG).show();
						Looper.loop(); // 执行消息队列中的消息
					}
				} catch (Exception e)
